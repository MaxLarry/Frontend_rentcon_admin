...
function PendingRequestdash() {
  ...
  return (
    <div className="bg-white border border-gray-200 p-5 rounded-md shadow-md block items-center lg:col-start-1 lg:col-end-4 lg:row-start-2 lg:row-end-5 md:col-start-1 md:col-end-3 relative">
      <div className="flex justify-between mb-4">
        <span className="text-lg font-bold text-zinc-900">Pending Requests</span>
        ...
        <table className="min-w-full bg-white text-center text-sm">
          <thead>
            <tr className="border-b">
              <th className="px-6 py-2 text-gray-700 cursor-pointer" onClick={() => handleSort('id')}>
                ID
              </th>
              <th className="px-6 py-2 text-gray-700 cursor-pointer" onClick={() => handleSort('requesterName')}>
                Requester Name {sortConfig.key === 'requesterName' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
              </th>
              <th className="px-6 py-2 text-gray-700 cursor-pointer" onClick={() => handleSort('date')}>
          ...
          <tbody>
            {currentRequests.length > 0 ? (
              ...
              <tr>
                <td colSpan="4" className="text-center p-4">No requests found.</td>
              </tr>
            )}
          </tbody>
        </table>
