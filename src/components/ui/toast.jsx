...
const ToastViewport = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-20 right-0 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-auto sm:top-15 sm:flex-col md:max-w-[420px]",
...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
  return (
    ...
    >
      {variant === "destructive" && (
        <div className="relative flex h-6 w-6 items-center justify-center rounded-full text-white text-xs font-bold">
          <CircleAlert />
        </div>
      )}
      {variant === "success" && ( // Render big check icon for success variant
        <div className="relative flex h-6 w-6 items-center justify-center rounded-full text-green-600 text-xs font-bold">
          <CircleCheckBig />
        </div>
      )}
      <div className="flex-1">{props.children}</div>{" "}
      {/* Ensure this holds the description */}
    </ToastPrimitives.Root>
  );
});
